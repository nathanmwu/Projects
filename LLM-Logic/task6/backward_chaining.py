class BackwardChainer:
    ...
    def backward_chain(self, goal, depth=0):
        ...
        indent = "  " * depth * 4  # Create indentation based on recursion depth
        
        print(f"{indent}PROVE: {goal}")
        
        # 1. check if goal is directly asserted
        if goal in self.assertions:
            print(f"{indent}✓ Goal is in assertions")
            return True
        
        print(f"{indent}Goal not directly asserted, searching rules...")
        ...
        for rule in self.rules:
            ...
            if bindings is not None:
                print(f"{indent}Found rule with matching consequent: {consequent}")
                print(f"{indent}Bindings: {bindings}")
                
                antecedent_sub = [self.substitute(a, bindings) for a in antecedent]
                print(f"{indent}Need to prove: {antecedent_sub}")
                
                all_proven = True
                for a in antecedent_sub:
                    if not self.backward_chain(a, depth + 1):
                        all_proven = False
                        break
                ...
                if all_proven:
                    ...
                    print(f"{indent}✗ Failed to prove all antecedents for {goal}")
        
        # 3. if no rules apply, goal is assumed false
        print(f"{indent}✗ No applicable rules found for {goal}")
        return False
