...
bool equals(List A, List B){
    ...
    N = A->front;   // temp nodes iterated through list
    M = B->front;
    while( eq && N!=NULL){
        eq = ( N->data==M->data );
        N = N->next;
...
List copyList(List L){
    // Returns a new List representing the same integer sequence as L. The cursor in the 
    // new list is undefined, regardless of the state of the cursor in L. The state of L is unchanged.
    if( L==NULL ){
      fprintf(stderr, "List Error: calling copyList() on NULL List reference\n");
      exit(EXIT_FAILURE);
    }
    List M = newList();     // new List to be returned
    Node walk = L->front;
    while (walk!=NULL){     // walks through every node in L
        append(M, walk->data);
        walk = walk->next;
    }
    return M;
