...
// becomes undefined.
// Pre: contains(k).
void Dictionary::reg_remove(keyType k){
    ...
    if(z != nil){   
        ...
        std::string error_msg = "Dictionary: reg_remove(): key \"" + k + "\" does not exist";
...
void Dictionary::LeftRotate(Node* N){
    ...
    N->right = y->left;
    if(y->left != nil){ // not necessary if using sentinal nil noder
        y->left->parent = N;
    }
...
void Dictionary::RightRotate(Node* N){
    // set y
    Node* y = N->left;
    // turn y's left subtree into N's left subtree
    N->left = y->right;
    if(y->right != nil){ // not necessary if using sentinal nil noder
        y->right->parent = N;
    }
    // link y's parent to N
    y->parent = N->parent;
